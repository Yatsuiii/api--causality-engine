...
use crate::jsonpath;
use crate::variables::resolve_template;
...
async fn run_hooks(hooks: &[Hook], context: &mut HashMap<String, String>, allow_delay: bool) {
    for hook in hooks {
...
fn extract_variables(step: &Step, response: &HttpResponse) -> HashMap<String, String> {
    ...
    for (var_name, path) in extract_map {
        if let Some(ref json) = json
            && let Some(value) = jsonpath::extract_string(json, path)
        {
...
mod tests {
    ...
    fn auth_resolves_templates() {
        ...
        ctx.insert("token".into(), "resolved-token".into());
        apply_auth(&auth, &mut headers, &ctx);
        assert_eq!(
    ...
    fn extract_from_json_response() {
        ...
        let extracted = extract_variables(&step, &response);
        assert_eq!(extracted.get("user_id").unwrap(), "42");
        assert_eq!(extracted.get("name").unwrap(), "Alice");
    }
